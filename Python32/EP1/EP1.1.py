...
for i in txtB:
    if len(i) == 7 :
        if  i[0] in 'plmrs' :
            if'x' not in i:
                    palavra.append(i)
print("temos %d preposições " % len(palavra))
...
for i in txtB:
    if len(i) >= 5 :
        if  i[-1] in 'plmrs' :
                    verbo.append(i)
print("temos %s verbos" % len(verbo))
primeira=[]
for i in txtB:
    if len(i) >= 5 :
        if  i[-1] in 'plmrs' :
            if len(i)%2==0:
                    primeira.append(i)
print("temos %s verbos em primeira pessoa"% len(primeira))
...
duplas = sorted(wc.items(), key=cont, reverse=True)
