...
def x_antes(words):
  l=[]                    #outra forma de fazer
  l1=[]                   #xantes = []
  for x in words:         #resto = []
    if x[0]=='x':         #for item in words
      l.append(x)         #if item.starwisth('x'):
    else:                 # xantes.append(item)
      l1.append(x)        #else:
  l.sort()                  #resto.append(item)
  l1.sort()               #return sorted(xresto)+sorted(item)
  return l+l1
